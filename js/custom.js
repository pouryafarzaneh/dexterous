...
$(document).ready(function() { 
    ...
    form.validate({
        ...
        submitHandler: function(form) {
          ...
          var m = form.method;
          var message = createContactMessage(getFormDataObject(form));

          $.ajax({
            url: actionUrl+'?message=' + message,
            method: m, 
            headers: {
              "x-message": message
            },
          ...
          }).then(function(responseText, statusText, xhr)  { 
            var messageId = responseText.PublishResponse.PublishResult.MessageId;
            if (messageId) contactFormEl[0].reset();
            contactFormEl.children('.messagebox').html('Thanks for your message! We will get in touch shortly!');
